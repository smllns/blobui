...
async function installRegistryComponent(name, index, installedComponents, downloadedFiles, installedDependencies, config, options) {
    if (installedComponents.has(name)) {
        return;
    }
    const registryItem = await getRegistryComponent(name, index);
    const componentExists = await checkComponentExists(registryItem.files, config);
    ...
    for (const dependency of registryItem.registryDependencies ?? []) {
        await installRegistryComponent(dependency, index, installedComponents, downloadedFiles, installedDependencies, config, options);
    }
...
export async function addComponent(name, options = {}) {
    ...
    await installRegistryComponent(name, index, installedComponents, downloadedFiles, installedDependencies, config, options);
}
