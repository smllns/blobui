...
export async function updateViteConfig() {
    ...
    if (!configPath) {
        console.log(`${colors.warning('⚠')} vite.config not found, skipping alias setup`);
        return;
    }
    ...
    if (!content.includes("from 'node:url'")) {
        content = content.replace(/import\s+\{\s*defineConfig\s*\}\s+from\s+['"]vite['"];?/, (match) => `import { fileURLToPath, URL } from 'node:url';\n${match}`);
    }
